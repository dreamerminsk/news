...
for feed in feeds.find():
    ...
    if diff.seconds > 900:
        ...
        for channel in root.findall('channel'):
            feeds.update_one({'link': feed['link']}, {'$set': {'title': item.find('title').text}}, upsert=False)
            feeds.update_one({'link': feed['link']}, {'$set': {'description': item.find('description').text}}, upsert=False)
...
for item in total:
    if 'thread' not in item:
        ...
        if 'tut.by' in item['link']:
            ...
            dt = datetime(1, 1, 1, 0, 0, 0) if t is None else datetime.strptime(t.get('datetime'), '%Y-%m-%dT%H:%M:%S%z')
